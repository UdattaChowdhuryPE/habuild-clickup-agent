...
5. Build two output strings:
   ...
   - `description_violation_details` = semicolon-separated `[task_name] (task_id)` pairs (or empty string if 0)
...
6. Build two output strings:
   ...
   - `ac_violation_details` = semicolon-separated `[task_name] (task_id)` pairs (or empty string if 0)
...
Where "task details" format is: `[task_name] (task_id)` with semicolons separating multiple entries.
...
description_violations: Login Flow (123abc); Payment Widget (456def); Dashboard Widget (789ghi)
ac_eligible_count: 9
ac_missing_count: 2
ac_violations: Payment Widget (456def); Config Panel (999zzz)
```
